...
hyperparameters:
+ batch_size: размер пакета, на которые мы делим тренировочный сет, то есть количество тренировочных объектов, которые обрабатываются перед обновлением модели за одну итерацию градиентного спуска (функция для уменьшения значения функции затрат)
